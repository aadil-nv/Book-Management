...
export const registerUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(HttpStatusCode.CREATED).json({ message: AUTH_MESSAGES.USER_REGISTERED , user , accessToken ,refreshToken});
    } catch (error) {
      next(error);
    }
  };
...
export const loginUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(HttpStatusCode.OK).json({ message: AUTH_MESSAGES.USER_LOGGED_IN ,username:user.username , accessToken ,refreshToken });
  } catch (error) {
    next(error);
  }
};
